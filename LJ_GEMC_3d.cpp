//--------------------------------------------------------------------------------------------------------------------------
//	GEMC code written for the system of Lennard-Jones particles
//	Based on algorithms from the text book "Understanding Molecular Simulations (Frenkel and Smit)"
//  Under guidance of Dr. Ethayaraja Mani (Ph.D. Advisor)
//	Written by Remya Ann on 28/11/2017 and modified on 28/2/2019
...
double	 delta       	= M_PI/2.;	//chi*M_PI;	//acos(1.0-(2.*chi)); //chi is patch coverage		//M_PI/2.;
double	 chi	     	= delta/M_PI;
int	 nadj      	= 5000;
int	 nsample_e 	= 10000; 
int	 nsample_p 	= 10000; 
int 	 eqconfig  	= 2000;
...

{
	...
	for(int iann=0; iann<anncyc; iann++)
	{
		...
		volmov   = 2;
		swapmove = 1000;//10;
		nmov     = npart_tot + swapmove + volmov;		//Number of moves in each 
		readparameterscheck();
...
void readparameters()
{
	...
	V[0]      = double(npart[0])/rho[0];	//pow(box[0],2.);
	V[1]      = double(npart[1])/rho[1];	//pow(box[1],2.);
	box[0]    = pow(V[0],1./3.);
	box[1]    = pow(V[1],1./3.);
...
void initialcondition(int start, int a) // a = box index
{
	...
	if (start == 1)					//Random initial configuration
	{
		...
		for (int i=first+1; i<last; i++)		//Coordinates of rest of the particles with overlap check
		{
			...
			}   while (r1<1.0);		//Hard Sphere diameter is 1.0
...
//		patch[i] = newpatch(patch[i], 0.7854);
...
//			r1  = rij.norm();
			if (r2 < cut2)
			{
				inv2	    = 1./r2;
				inv6	    = inv2*inv2*inv2;
				if (shift){toten+= (4.*inv6*(inv6-1.)) - Ucut;}
				else	   {toten+=  4.*inv6*(inv6-1.);}
				W[boxind] += 48.*inv6*(inv6-0.5);
			}
		    }
		}
	    }
...
//					
//--------------------------------------------------------------------------------------------------------------------------
double potenergy(int j, vector rn, int boxind)
{
	double en = 0.0;
	double r2, inv2, inv6;
	vector rij;
	...
	for (int i=0; i<npart_tot; i++)
	{
		...
		if (i != j)
		{
			...
			if (r2 < cut2)
			{
				inv2	 = 1./r2;
				inv6	 = inv2*inv2*inv2;
				if (shift){en+= (4.*inv6*(inv6-1.)) - Ucut;}
				else	   {en+=  4.*inv6*(inv6-1.);}
				Wi	+= 48.*inv6*(inv6-0.5);
			}
		}
	    }
...
//	vectadd(ro, dr ,rn) 				// rn = New configuration
	rn       = ro + dr;
//	pn       = newpatch(po, 0.05);			// pn = New orientation
